...
ffmpeg -y -loglevel error -framerate $FPS -i "$WORK/frames/f%04d.png" -i "$WORK/track.wav" \
  -vf "noise=alls=2:allf=t,format=yuv420p" -c:v libx264 -crf 20 -preset medium \
...
res = "NO FRAME"
if ok:
    g = cv2.cvtColor(fr[695:1180, 300:785], cv2.COLOR_BGR2GRAY)  # QR card @1080x1920 (IG default)
