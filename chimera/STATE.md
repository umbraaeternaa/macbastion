...
Last completed: **`chimera status` surfaces the reactive state — the one mind, visible** (commit `00638d6`).
`core.status` gained a `reactive` section: the live PULSE cognitive mode (`pulse_mode`, default `normal`)
and the armed cross-module reflexes (data-driven `RELAY_RULES` + the two code reflexes:
`pulse:exhausted -> vault.lock`, `tether.escalation L1/L2 -> shim.lock/vault.lock`); `render_status` prints
them under the module table. `chimera status` now shows the organism's reactive intelligence at a glance —
cognitive mode + which defenses are wired, not just a process list. The demo bridge for the project's core
differentiator (research §4). 5 RED->GREEN (614 default). 980 -> 985. mypy --strict + ruff clean. NEXT:
vault open-state in status (needs a vault.status query), a swiftbar passive surface, or a new slice.
