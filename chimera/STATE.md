...
Last completed: **PURGE tier0 real state-file removal — LIVE (CHIMERA-own-file registry, `src/statefiles.c`)**
(Day 21 — PURGE's real hands, key-first §3: tier0 after tier3). The tier executor (`src/exec.c`) now wires a
REAL default tier0 action: `purge_statefile_remove_all()` (`src/statefiles.c`) `unlink()`s every registered
absolute path (register refuses NULL/empty/non-absolute/over-long, caps at 32; static table — no allocation on
the purge path). `purge_statefile_register(path)` (C API) lets CHIMERA register its OWN state DBs/files; an armed
`purge.trigger` now REALLY removes them from disk. tier0's Keychain-eviction half and tier1 (VAULT KEK crypto-
shred) remain honest NO-OP stubs (§4). 4 registry tests (real /tmp deletion) + 1 exec integration test
(`937096c`); native PURGE 49 → 54 (total 1102 → 1107). Tested on THROWAWAY data only — NOT live-armed.
Prior (`c0193ea`, `086f746`): tier3 real RAM-wipe LIVE — `purge_secret_clear_all()` `purge_wipe()`s registered
sensitive buffers (RAM only, nothing on disk). NEXT slices (careful, THROWAWAY data, live-arm only on explicit
operator opt-in): tier0's Keychain-evict half via core→shim (`shim.evict`, authorized); tier1 VAULT KEK crypto-shred.
