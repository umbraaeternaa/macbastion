...
Then live-pf validation (Day 22): the dummynet recipe was PROVEN on the live kernel — `dnctl pipe 1337 config bw 2Mbit/s` + `dummynet in all` cut download ~5.1MB/s -> 229831 B/s (~1.84Mbit, at the cap); `dnctl show` confirmed traffic + drops through pipe 1337; clean restore to stock. Recorded in `docs/ECHO_PACKET.md` §7 with two findings: F1 a parent `dummynet-anchor "com.chimera.echo"` hook in the MAIN ruleset is REQUIRED (a bare sub-anchor is inert; nesting under stock `com.apple/*` did NOT carry it), F2 shaping needs BOTH `in`+`out` (the candidate was out-only). NEXT: ratify the §2 never-list narrow exception (shaper adds ONE hook line to main, superset load + FAIL-OPEN restore via `pfctl -f /etc/pf.conf`), then TDD-rewrite `anchor.c` to the proven recipe (root-free, via the injectable seam). Distribution also shipped this session: `setup.sh` one-command clone-and-run
