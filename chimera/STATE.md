...
injectable backend, FAIL-OPEN, root-free recording tests (6) — echo-shaper now 11. The real pfctl/dnctl execution
and a validated dummynet rule = the NEXT gated slice, per-command root approval. Distribution also shipped this session: `setup.sh` one-command clone-and-run
