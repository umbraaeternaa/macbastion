...
pf/root (§4); 5 Unity tests; wired into check.sh. The real pf path = later gated slices on a throwaway anchor,
each with per-command root approval. Distribution also shipped this session: `setup.sh` one-command clone-and-run
