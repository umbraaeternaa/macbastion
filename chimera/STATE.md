...
Last completed: **TETHER-loss auto-locks the open vault — the "one mind"** (commit `696e2e8`). First
cross-module reflex: a new `RELAY_RULES` entry maps `tether.absent` (TETHER dead-man — paired phone out of
range) -> `vault.lock`. When the broker publishes `tether.absent`, core (as authority, in-process, D7 wire
guard untouched) issues `vault.lock`, closing the open vault + tearing down its RAM plaintext mount
(VD-9b). Data on the proven anomaly-tripwire relay path; mirrors `oracle.anomaly.detected -> tether.heighten`.
1 RED->GREEN (596 default). 965 -> 966. This is CHIMERA's emergent-behaviour showcase (research §4 — "one
mind"). NEXT: more reflexes (e.g. PULSE-fatigue gating, ORACLE-anomaly -> vault.lock), or live-wire e2e.
