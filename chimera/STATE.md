...
Last completed: **PURGE tier3 real RAM-wipe — LIVE (sensitive-region registry, `src/secrets.c`)**
(Day 21 — the last module gains real hands; tier3 first, builds on the executor skeleton `0ad29b1`). The tier
executor (`src/exec.c`) now wires a REAL default tier3 action: `purge_secret_clear_all()` (`src/secrets.c`)
`purge_wipe()`s every registered sensitive buffer (optimization-resistant, dc-zva accelerated; the registry is
static — no allocation on the purge path). `purge_secret_register(buf,len)` (C API) lets CHIMERA register its
own key material (KEKs, the per-boot secret, plaintext); an armed `purge.trigger` now REALLY zeroes those
buffers in RAM. SAFE — touches ONLY registered process memory: nothing on disk, nothing of the operator's.
tier0 (Keychain evict) and tier1 (VAULT KEK crypto-shred) remain honest NO-OP stubs (§4). `purge.trigger` stays
armed-gated + single-shot (unarmed → `-31004`). 4 registry tests + 1 exec integration test (real RAM zeroing
through the executor, `c0193ea`); native PURGE 44 → 49 (total 1097 → 1102). NEXT slices (careful, THROWAWAY data,
live-arm only on explicit operator opt-in): tier0 Keychain evict via core→shim (`shim.evict`, authorized) +
CHIMERA state-DB removal; tier1 VAULT KEK crypto-shred.
