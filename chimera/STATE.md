...
Last completed: **ECHO packet-shaper — §8 Amendment A1 ratified + `chimera/echo-shaper/` skeleton LIVE**
(Day 21). ECHO's real constant-rate normalization needs packet-plane root, which the shim cannot carry (§8.8:
never opens sockets). A1 (`docs/ECHO_PACKET.md`, LOCKED) admits ONE narrow new domain — a SEPARATE
`chimera-echo-shaper` root daemon (NOT the shim), opt-in, FAIL-OPEN, EP-1…8. First gated slice: the no-op
skeleton — op whitelist (shaper.anchor.load / .pace / .anchor.remove) + a FAIL-OPEN no-op executor touching no
pf/root (§4); 5 Unity tests; wired into check.sh. Then the pf-anchor lifecycle (`anchor.c`): build-rules + apply/clear via an
injectable backend, FAIL-OPEN, root-free recording tests (6) — echo-shaper now 11. Then EP-5 dispatch (`protocol.c`):
shaper.* JSON-RPC routing, auth-first + per-boot-secret gate on the pf ops (root-free; +cJSON/jsonrpc) — echo-shaper now 17.
...
Then live-pf validation (Day 22): the dummynet recipe was PROVEN on the live kernel — `dnctl pipe 1337 config bw 2Mbit/s` + `dummynet in all` cut download ~5.1MB/s -> 229831 B/s (~1.84Mbit, at the cap); `dnctl show` confirmed traffic + drops through pipe 1337; clean restore to stock. Recorded in `docs/ECHO_PACKET.md` §7 with two findings: F1 a parent `dummynet-anchor "com.chimera.echo"` hook in the MAIN ruleset is REQUIRED (a bare sub-anchor is inert; nesting under stock `com.apple/*` did NOT carry it), F2 shaping needs BOTH `in`+`out` (the candidate was out-only). Then (Day 22) EP-9 ratified (`docs/ECHO_PACKET.md`: the required parent hook is added install-time, runtime touches only our anchor) + `anchor.c` F2 fix: `build_rules` now emits BOTH `dummynet in all` + `dummynet out all` (was out-only; CANDIDATE -> VALIDATED comments), echo-shaper still 24. Then (Day 22) the EP-9 install-time hook installer shipped: `deploy/install-echo-shaper.sh` (mirrors install-shim.sh) installs the binary + LaunchDaemon plist AND adds `dummynet-anchor "com.chimera.echo"` to `/etc/pf.conf` (idempotent, pre-edit backup; `--uninstall` pulls it back out); the pf.conf-hook logic is tested ROOT-FREE via `chimera/tests/test_echo_shaper_install.py` (3 pytest, `PF_CONF`-overridable). Then (Day 22) C1 socket-ownership seam: `ownership.c` (`shaper_ownership_apply` — chown root:operator_group + chmod 0660, mirrors shim SS-0(b)) wired into `main.c` under `-m privileged` (off by default; the installer plist now passes it), so the non-root core can reach the root daemon's socket; 2 root-free tests (NULL + non-root EPERM, success path manual-tier) — echo-shaper 24 -> 26. Then (Day 22) C2a `shaper.handshake`: hands the per-boot secret to the authorized (peercred=operator) core — peercred-only, ratified as EP-10 (no SecCode attestation; EP-8 low-stakes + the shaper's no-framework minimalism); +2 protocol tests, echo-shaper 26 -> 28, live-smoke returns a 64-hex secret. Then (Day 22) C2b: `core/echo_shaper_client.py` (`EchoShaperClient`, mirrors `shim_client.py` — connect -> `shaper.handshake` caches the per-boot secret -> `shape(rate)`/`unshape()` carry it on the pf ops), 6 root-free pytest (fake unix server on a short /tmp path, default suite), Python 694 -> 700. Then (Day 22) C2c: `core/server.py` gains `core.echo.shape`/`core.echo.unshape` operator commands forwarding to an injected `EchoShaperClient` (mirrors `core.lock`->shim; opt-in — absent client -> PRECONDITION_FAILED, bad rate -> INVALID_PARAMS), 4 root-free pytest (fake client object), Python 700 -> 704. Then (Day 22) S startup-injection: `CoreConfig.echo_shaper_enabled` (opt-in, OFF by default, EP-4) -> `build_core` wires a real `EchoShaperClient` so `core.echo.shape`/`unshape` reach the daemon in production; 2 root-free pytest, Python 704 -> 706. **ECHO-shaper integration end-to-end COMPLETE** (build -> validated live shaping -> installer -> ownership -> handshake -> client -> core commands -> opt-in startup-wiring). Then (Day 22) slice D: the pf-op secret gate is now constant-time — `protocol.c` uses `shaper_secret_equal` over the full 64 hex chars + a length guard (was `strcmp`); protocol tests moved to a 64-hex secret + a wrong-length-rejected case, echo-shaper 28 -> 29. End-to-end live smoke: `EchoShaperClient` -> handshake (64-hex) -> `shape()` whose secret PASSES the gate (the op then fails only on non-root pfctl). Then (Day 22) EP-6 DESIGNED + DEFERRED (`docs/ECHO_PACKET.md` §8): floor-filling is a research-grade `dnctl`->deficit->CHAFF feedback loop; the rate CEILING (the main volume-fingerprint win) is already in-kernel, while the FLOOR burns bandwidth continuously + its benefit is threat-model-dependent, so deferred honestly (§4) rather than half-shipped (`shaper.pace` stays an explicit no-op). **ECHO packet-shaper arc CLOSED** — a complete, live-validated, installable, hardened organ (11 slices, Day 22: main.c -> live pf validation -> EP-9 -> F2 -> installer -> C1 ownership -> C2a handshake -> C2b client -> C2c commands -> S startup-injection -> D constant-time gate). RECON (Day 22) — the organism is ALREADY LIVE: shim (root daemon) + core + all 8 modules up ~21.5h via the installed LaunchAgent `com.umbra.chimera.plist` (which runs the FROZEN `dist/` binary — stale, pre-Day-22). INCIDENT: the live echo-shaper deploy hit `install-echo-shaper.sh`'s live `pfctl -f /etc/pf.conf`, which FLUSHED dynamically-inserted system pf anchors and DROPPED the operator's Wi-Fi; recovered by a reboot. FIX (this commit): the installer NO LONGER live-reloads pf — the hook is a `/etc/pf.conf` file edit that activates at the NEXT REBOOT (EP-9 updated with the hard lesson; NEVER `pfctl -f` the main ruleset on a live machine). The leftover echo-shaper daemon + inert (empty) hook remain installed, harmless (an empty anchor shapes nothing), until cleaned (no-reload: `cp /etc/pf.conf.chimera-bak /etc/pf.conf` + bootout, effective at reboot) or redeployed. NEXT: operator-directed. Distribution also shipped this session: `setup.sh` one-command clone-and-run
...
(Day 21 — PURGE's real hands, key-first §3; the last destruction tier with real effect). `purge_execute` now
takes the Tier-2 target registry and runs tier0 -> tier1 -> tier2 -> tier3. For each operator target
(`purge.target.add`): ENCRYPTED -> `unlink` (reliable — ciphertext is noise once keys are shredded by tier0/1);
UNENCRYPTED -> REFUSED + counted in `tier2_skip` (NO SSD-overwrite theatre, §8). Removal runs through an
injectable per-target seam; the real default is `unlink` (ENOENT counts as removed, best-effort). `purge.trigger`
passes its targets + returns `tier2_skip`. 2 exec integration tests (encrypted target really removed, unencrypted
left UNTOUCHED; `b574884`); native PURGE 54 → 56 (total 1107 → 1109). Tested on THROWAWAY /tmp only — NOT live-armed.
Prior this session (`c0193ea`/`937096c`): tier3 real RAM-wipe (secrets registry) + tier0 real state-file removal
(statefiles registry) LIVE. The ONLY remaining no-op is native tier1 VAULT KEK crypto-shred — but it is ALREADY
covered at the operator level by `core.purge` -> `shim.evict`, which evicts ALL `com.umbra.chimera` Keychain items
(every VAULT KEK lives under that service, `keychain.c:14`). So the destruction system is COMPLETE end-to-end:
core does Keychain + KEK + state-wipe + module-SIGKILL; the native organ does files + RAM + operator Tier-2 targets.

Prior milestone: **ORACLE reliable scoring + autonomous defense LIVE on the frozen organism; v0.3.0-alpha released**
(Day 20→21 — Phase 3 capstone). ORACLE's llama3.2:1b reasoned correctly but output score 0.0 for blatant
ransomware (then, with an over-aggressive prompt, 0.9 for benign) — the 1B model can't calibrate. Fix
(`f0ead42`): a balanced scoring rubric + few-shot anchors in the classify prompt, and default model switched to
**qwen2.5:7b** (already installed; `CHIMERA_ORACLE_MODEL` overrides — wired in `oracle/__main__`). Calibrates
cleanly: ransomware → 1.0, Safari/​git → 0.05, Tor-exfil → 0.9.
✅ **LIVE end-to-end on the frozen organism**: ransomware classify (0.97) → `oracle.anomaly.detected` → 4 real
reflexes [ok] (vault.lock + chaff.generation.start [REAL libcurl] + echo.start + tether.heighten); then a
benign event → `oracle.anomaly.cleared` → chaff/echo stood down. Reliable autonomous defense, real effectors.
**v0.3.0-alpha** tagged + GitHub pre-release ("the organism acts"). 691 default tests (cli 18→22: frozen-aware
`_chimera_root`/`_module_python`), total 1089 → **1093**.
⚠️ **Honest incident (not a CHIMERA bug)**: mid-testing the Mac kernel-panicked — `userspace watchdog timeout:
no WindowServer checkins in 120s` (macOS graphics-server starved). Cause: operator-side test STORM (several
frozen test cores + orphaned modules + both 1b and 7b models loaded) overloaded the machine; the shim did NOT
reboot (logs empty), audit was clean, nothing lost, git in sync, organism auto-restarted 8/8. Lesson logged:
clean up test processes immediately, never load two models, no test storms on the live machine.

Prior milestone: **FROZEN SIGNED CORE is the LIVE ORGANISM — shim attestation PASSES, destructive ops UN-GATED**
...
✅ **LIVE THREAT REFLEX FIRED on the running 8/8 organism** (Day 19): wired ORACLE's LLM `Detector(LlmClient(),
store)` into its daemon (`oracle/__main__.py`) so `oracle.classify` works live via local Ollama (llama3.2:1b).
A throwaway client fed the live organism a blatant ransomware event -> the LLM scored it 0.5 -> ORACLE emitted
...
(`modules/NAME/NAME`); a Python module (`ModuleSpec.python=True`, new field) runs as `[sys.executable, -m,
NAME]` with its package dir on PYTHONPATH. `_default_spawn` now uses it. Unblocks oracle/pulse (Python pkgs,
not binaries) for supervised bring-up. 2 RED->GREEN (12 cli). 1076 -> 1078. mypy --strict + ruff clean.
**✅ ORGANISM IS LIVE ON THE MACHINE (Day 19):** `python -m core up` (CHIMERA_SOCKET_DIR=/tmp/chimera-run)
brought the real organism up — `chimera status` showed core v0.2.0-alpha + echo + purge registered, PULSE
normal, and the **6 armed reflexes** (the one mind: tether.absent->vault.lock, oracle.anomaly->fan-out,
pulse:exhausted->vault.lock, escalation L1/L2, 2 stand-downs). Not a demo — a real running process. vault
binary built this session. NEXT (DP-2): extend `CHIMERA_MODULES` (echo+purge only today) with all buildable
organs + their depends_on (chaff, mirror, vault native; oracle, pulse python=True) so one `chimera up` raises
the FULL organism; then live-verify a reflex firing (tether.absent -> vault locks).

Prior milestone: **PURGE `dc zva` ARM64 acceleration in `purge_wipe` (PW-2)** (commit `a3cec35`, Day 18). New
`src/asm/wipe_zva.S` (AArch64): `purge_zva_blocksize()` reads `DCZID_EL0` (4<<BS bytes, or 0 if DZP prohibits
dc zva) + `purge_zva_one()` zeroes one block via `dc zva`. `purge_wipe` rewritten: byte-wipe the unaligned
head -> `dc zva` the aligned middle -> byte-wipe the tail (falls back to the pure byte loop if dc zva is
unavailable). PURGE's Makefile now builds `.S` (ASM_OBJS, like MIRROR). The first ARM64 asm in PURGE — the
manifesto's "asm where it matters" (§5.8). Live on the M2: the blocksize test asserts a sane power-of-two from
DCZID_EL0 and a 1024-byte buffer zeroes through the dc-zva path. 2 RED->GREEN Unity (40 PURGE). 1074 -> 1076.
-Werror clean (incl asm). NEXT (PW-3): wire `purge_wipe` into the gated `purge.trigger` path (still operator
opt-in). BIG NEXT FRONTIER (tracked): deploy the organism live on the operator's machine, once a clean point.

Prior milestone: **PURGE secure-wipe core — `purge_wipe` (PW-1)** (commit `54817ec`, Day 18). New
`modules/purge/{include/wipe.h,src/wipe.c}`: `purge_wipe(buf, len)` zeroes memory through a `volatile`
pointer so the wipe is NOT dead-store-eliminated even when the buffer is freed right after (the point of a
secure wipe); NULL/len==0 are no-ops. The guaranteed-correct C core of §5.8 RAM-erase — the first real PURGE
destruction primitive (the planner/registry/dispatch existed; `purge.trigger` is still gated -31004). 3
RED->GREEN Unity (38 PURGE). 1071 -> 1074. -Werror clean. SAFE to build: PURGE's destructive trigger is
operator opt-in (never auto-fires), so this is the mechanism, not a loaded gun. NEXT (PW-2): ARM64 `dc zva`
(data-cache-zero-by-VA) acceleration on top — add the asm build to PURGE's Makefile + a .S routine (align
head/tail in C, `dc zva` the aligned middle), behind `purge_wipe`. Then wire real wipe into the gated
`purge.trigger` path (still opt-in). BIG NEXT FRONTIER (tracked): deploy the organism live on the operator's
machine (LaunchAgent + bring-up + see it protect), once a clean stopping point.

Prior milestone: **PULSE consumes ORACLE anomaly as group-C drift — ALL 3 SENSES LIVE (PD-C-2)** (commit
`fcdd00c`, Day 18). PULSE subscribes to `oracle.anomaly.detected`; `_on_event_frame` dispatches by method
(mirror.input.minute -> group-A, oracle.anomaly.detected -> `DriftTracker.observe(score)`); `_compute` passes
...
`oracle.anomaly.detected`, already emitted) as the group-C behavioral-drift fatigue signal for a 5-min window
(`DRIFT_WINDOW_S`), then decays to absent; only a meaningful anomaly (`> DRIFT_MIN` 0.3) counts. Pure/hermetic
(score + now injected); `assess()` consumes `drift()`. The 3rd PULSE sense — the producer already exists, so
group-C is PULSE-side only (mirror of the idle arc). 5 RED->GREEN. 1063 -> 1068. mypy --strict + ruff clean.
NEXT (PD-C-2): wire the daemon — add `oracle.anomaly.detected` to `SUBSCRIBE_TOPICS`, `_on_event_frame`
extracts `score` -> `DriftTracker.observe`, `_compute` passes `drift=self._drift.drift(now)` to assess. Then
all THREE PULSE senses are live (input group-A + temporal/idle group-B + ORACLE drift group-C).

Prior milestone: **MIRROR real passive CGEventTap input observer — group-A LIVE end-to-end (MI-5b)** (commit
...
caller holds rt->mutex. 4 RED->GREEN Unity (63 MIRROR). 1059 -> 1063. -Werror clean. NEXT (MI-5b, the last
group-A link): the real passive CGEventTap (manual-tier, same Accessibility TCC as the injector, no new dep) —
a listen-only tap on keyDown + mouseMoved on its own run-loop thread, extracting keycode / dx,dy and calling
`mirror_observe_event` under the lock + wiring `mirror_tick_input_minute(rt, time(NULL))` into the daemon loop.
Then group-A is LIVE end-to-end (real typing/mouse -> PULSE fatigue -> gate friction); capstone w/ live-verify.

Prior milestone: **MIRROR per-minute group-A emit driver (MI-4)** (commit `4857f07`, Day 18). The runtime gains
a live `input` aggregator (`rt->input`) + `input_minute_at` window stamp (zeroed in `mirror_runtime_init`).
`mirror_tick_input_minute(rt, now)` arms the window on the first call (no emit), then once
`MIRROR_INPUT_MINUTE_S` (60s) elapses it `inputagg_roll`s `rt->input` + emits `mirror.input.minute` + re-arms
(1 on emit). `now` injected -> unit-testable; the daemon will pass `time(NULL)`. Kept to the tested driver —
the real passive CGEventTap feeding `rt->input` + the daemon-loop wiring land together in MI-5 so the live emit
ships with real data, not empty minutes. 3 RED->GREEN Unity (59 MIRROR). 1056 -> 1059. -Werror clean.
NEXT (MI-5, the last group-A link): a real passive CGEventTap (manual-tier, same Accessibility TCC as the
injector, no new dep) — keyDown classified char-vs-delete + mouseMoved dx/dy -> `inputagg` (raw discarded,
privacy §8) — plus wiring `mirror_tick_input_minute(rt, time(NULL))` into the daemon loop. Then group-A is
LIVE end-to-end (real typing/mouse -> PULSE fatigue -> gate friction); capstone with a live-verify like idle.

Prior milestone: **MIRROR `mirror.input.minute` event producer (MI-3)** (commit `b730930`, Day 18).
`mirror_emit_input_minute(rt, snap)` builds the per-minute group-A event `{chars, deletes, mouse_path_ratio}`
(ratio null when no mouse movement) from a rolled aggregate snapshot and enqueues the NDJSON line via a new
`evq_push` onto the runtime event queue — the producer seam `drain_events` was wired forward-compat for. PULSE
(PD-A-3) already subscribes to this topic. 2 RED->GREEN Unity (56 MIRROR). 1054 -> 1056. -Werror clean.
NEXT (MI-4): the real passive CGEventTap (manual-tier, same Accessibility TCC as the injector, no new dep) —
classify keyDown (char vs delete) + mouseMoved (dx/dy) -> feed `inputagg` (raw discarded, privacy §8) — plus a
per-minute timer in the daemon loop that `inputagg_roll`s + calls `mirror_emit_input_minute`. That closes
group-A end-to-end (real typing/mouse -> PULSE fatigue); then capstone with a live-verify like the idle loop.

Prior milestone: **MIRROR mouse path-ratio — the pure group-A data model is COMPLETE (MI-2)** (commit
`98ec570`, Day 18). `inputagg` gains mouse tracking: `inputagg_mouse_move` accumulates path length + net
displacement; `inputagg_mouse_ratio` = path / straight-line(net), the net floored at
`MIRROR_MOUSE_STRAIGHT_FLOOR` (1px) and the ratio capped at `MIRROR_MOUSE_RATIO_MAX` (10.0), pinned like
temporal.py's constants; 0.0 when no movement (-> absent). Completes the pure, testable group-A data model
(chars/deletes/mouse_ineff). Geometry pinned by tests (straight->1.0, right-angle->1.414, back-and-forth->cap).
Privacy §8 — counts + geometry only, never a coordinate stored. 5 RED->GREEN Unity (54 MIRROR). 1049 -> 1054.
-Werror clean. NEXT (MI-3): the real passive CGEventTap (manual-tier, same Accessibility TCC as the injector,
no new dep) feeding inputagg on each keystroke/mouse-move + a per-minute timer that rolls + emits
`mirror.input.minute {chars, deletes, mouse_path_ratio}` -> PULSE (already subscribed, PD-A-3). That closes
group-A end-to-end.

Prior milestone: **MIRROR per-minute user-input aggregator — the group-A producer begins (MI-1)** (commit
`acb272c`, Day 18). New `inputagg` (`include/inputagg.h` + `src/inputagg.c`), SEPARATE from `stats.c` (which
counts MIRROR's OWN injected events): counts the operator's printable `chars` + `deletes` per window;
`inputagg_roll(a, out)` snapshots the window then resets the live counters for the next minute. Privacy §8 at
the source — counts ONLY, never a key code / coordinate / payload. The first brick of MIRROR's passive input
sensor that will feed PULSE's dominant group-A signal (chars -> typing_speed, deletes/chars -> error_rate). 4
RED->GREEN Unity (49 MIRROR). 1045 -> 1049. -Werror clean. NEXT (MI-2): mouse path-length-vs-straight-line
ratio aggregator (mouse_ineff); then the real passive CGEventTap (manual-tier, same TCC as the injector, no
new dep) feeding inputagg + a per-minute timer emitting `mirror.input.minute` -> PULSE (already subscribed).

Prior milestone: **PULSE subscribes to MIRROR's input-aggregate event — PULSE side of group-A COMPLETE
(PD-A-3)** (commit `a2b7b1c`, Day 18). PULSE opens a 2nd connection (`events.sock`, consumer role like ORACLE),
`core.subscribe`s to `mirror.input.minute`, and folds each pushed aggregate into group-A via `_on_event_frame`
-> `_on_input_aggregates`. Defines the producer contract `{chars, deletes, mouse_path_ratio}` the MIRROR
observer will honor; sub-ack/unrelated events ignored; malformed payload ignored (fail-open). The PULSE side of
group-A is now COMPLETE (mapper PD-A-1 + feed PD-A-2 + subscription PD-A-3) — ready to consume the moment
MIRROR emits. 3 RED->GREEN + integration (daemon comes up with the live subscription). 1042 -> 1045. mypy
--strict + ruff clean. NEXT: the MIRROR producer C sub-arc — a passive CGEventTap -> per-minute counters
(chars/deletes/mouse path-ratio, privacy §8 aggregate-only, raw never stored) -> emit `mirror.input.minute`;
first slice = the pure C aggregator (Unity-testable), then the real tap (manual-tier), then emit.

Prior milestone: **PULSE feeds group-A into the fatigue score (PD-A-2)** (commit `c713980`, Day 18). The daemon
caches the latest MIRROR input aggregates via `_on_input_aggregates` (-> `input_signals`) and `_compute` now
passes `group_a=self._group_a` to `assess`, so the **dominant** group-A signal (w_a 0.55) folds into the score.
A quiet minute maps to absent (no fake fatigue). Tested: fatiguing input (error_rate 0.3 vs 0.05 baseline)
raises the score; an idle minute leaves it unchanged. The consume side of group-A is live; the source (MIRROR
aggregate event) is injected in tests. 2 RED->GREEN. 1040 -> 1042. mypy --strict + ruff clean. NEXT (PD-A-3):
events.sock subscription — PULSE subscribes to MIRROR's aggregate event -> `_on_input_aggregates` (new
subscription infra; PULSE has none yet — `client.py:6` "no events.sock consumer"); then the MIRROR passive
observer/producer C sub-arc (CGEventTap listen -> per-minute counters, privacy §8 aggregate-only).

Prior milestone: **PULSE group-A mapper — MIRROR aggregates -> fatigue readings (PD-A-1)** (commit `0442af6`,
Day 18). New pure `pulse/input.py` `input_signals()` maps MIRROR's per-minute aggregate counters (chars,
deletes, mouse path-ratio — privacy §8, never raw input) onto the three group-A signals `assess()` consumes:
`typing_speed` (chars), `error_rate` (deletes/chars), `mouse_ineff` (path ratio). A signal with no data this
minute is ABSENT, not 0 (a pause is not gradely-slow typing -> a quiet minute can't masquerade as peak
fatigue). The PULSE side of gap D8; MIRROR's real passive observer is a later sub-arc. 5 RED->GREEN. 1035 ->
1040. mypy --strict + ruff clean. NEXT (PD-A-2): wire the daemon — PULSE subscribes to MIRROR's aggregate
event -> `input_signals` -> cache -> feed `assess(group_a=...)` each tick (the strongest fatigue signal, w_a
0.55, goes live on the consume side; MIRROR's passive observer/producer is the following C sub-arc).

Prior milestone: **PULSE idle signal LIVE in the daemon (PD-idle-2)** (commit `f57d102`, Day 18). `_compute` no
