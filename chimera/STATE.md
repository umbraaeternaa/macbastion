...
Last completed: **`chimera status` — operator status-view of the live organism** (commit `6e06ca5`). First
operator-facing surface (UX.md: CLI = request/response). New `core/status_view.py` `render_status()` —
pure, deterministic render of the `core.status` payload (core header + one sorted row per module: status,
version, method count). New `chimera status` subcommand connects to `core.sock`, fetches `core.status`,
prints the view; a core that isn't running -> friendly message + exit 1, never a stack trace. Makes the
organism's state VISIBLE (was only in tests/logs) — the bridge from "works" to "showable" (research §4/§5).
5 render units + graceful-when-down + 1 integration (live socket). 973 -> 980. mypy --strict + ruff clean.
NEXT: richer status (PULSE mode, vault open, armed reflexes), a swiftbar surface, or a new slice.
