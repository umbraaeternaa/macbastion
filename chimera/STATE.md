...
Last completed: **vault.delete — registry drop + KEK eviction + content wipe** (commit `60e13a6`) — VD-7.
`vault.delete {vault_id}` removes a vault completely: drops it from `registry.json`, **evicts its
master-secret KEK from the Keychain** (new `del` backend hook; real path `SecItemDelete`, manual-tier),
unlinks its sealed-file index `<vault_id>.files.json`, and closes it if it is the open vault. `{ok:true}`;
unknown id -> `{ok:false, reason:no_such_vault}`. `vault.delete` un-gated; **`vault.policy.update` is now the
SOLE -31004 engine method**. 2 RED->GREEN Unity (65 VAULT). 954 -> 956. NEXT: `vault.policy.update`
(last gated method) and/or mount-to-tmpfs (manual-tier).
...
- Native (VAULT C Unity): 65 passing (6 lexer + 6 parser + 9 evaluator + 6 fail_closed + 3 relock + 7 decide + 7 crypto + 9 commands [VD-1 status + VD-2 create/list + VD-3 create-provisions-KEK + VD-7 delete-removes/no_such_vault + policy.update gated -31004] + 2 keychain [VD-3 load-or-create] + 10 unlock [VD-4a decision + VD-4b key-derive + VD-4c lock/auto-relock + VD-5 add_file seals + VD-6 decrypt-at-unlock round-trip]) — separate C suite, NOT in pytest. VAULT is a live daemon (VD-1..7): create provisions a per-vault Keychain KEK (-> evict has real targets); unlock is state-gated, derives the key + OPENS the sealed files (full add_file->unlock crypto round-trip); lock/auto-relock close the lifecycle; delete drops the vault + evicts its KEK + wipes content. `vault.policy.update` is the last gated method; mount-to-tmpfs is the remaining manual-tier step
