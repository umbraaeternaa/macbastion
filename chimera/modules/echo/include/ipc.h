/* UNIX-socket client to core (§6.3). NDJSON framing, poll-based reads. Mirrors CHAFF's
 * ipc, but returns plain int (0 ok / -1 error) — ECHO has no module result enum. */
#ifndef ECHO_IPC_H
#define ECHO_IPC_H
...
/* Send exactly len bytes (handles partial writes). 0 ok, -1 on I/O error. */
