/* Contract for JSON-RPC framing (§6.4). Adapted verbatim from chaff (Sub-M6) to
 * verify the chaff_->mirror_ rename of the copied jsonrpc.c didn't break it.
 * This exercises the REAL copied implementation, so these PASS in RED. */
#include "unity.h"
...
static void test_parse_request_ok(void) {
    ...
    const char *line = "{\"jsonrpc\":\"2.0\",\"id\":7,\"method\":\"mirror.status\"}";
    TEST_ASSERT_EQUAL_INT(MIRROR_OK, jsonrpc_parse_request(line, &req));
    TEST_ASSERT_NOT_NULL(req);
    TEST_ASSERT_EQUAL_STRING("mirror.status", req->method);
...
static void test_parse_notification_no_id(void) {
    jsonrpc_request_t *req = NULL;
    const char *line = "{\"jsonrpc\":\"2.0\",\"method\":\"mirror.tick\"}";
    TEST_ASSERT_EQUAL_INT(MIRROR_OK, jsonrpc_parse_request(line, &req));
    TEST_ASSERT_NOT_NULL(req);
...
static void test_parse_non_json_errors(void) {
    jsonrpc_request_t *req = NULL;
    TEST_ASSERT_EQUAL_INT(MIRROR_ERR_PARSE, jsonrpc_parse_request("not json", &req));
}

static void test_parse_missing_method_errors(void) {
    jsonrpc_request_t *req = NULL;
    const char *line = "{\"jsonrpc\":\"2.0\",\"id\":1}";
    TEST_ASSERT_EQUAL_INT(MIRROR_ERR_PARSE, jsonrpc_parse_request(line, &req));
}
...
static void test_serialize_notification_shape(void) {
    ...
    cJSON_AddStringToObject(params, "reason", "secure_field");
    char *out = jsonrpc_serialize_notification("mirror.profile.changed", params);
    TEST_ASSERT_NOT_NULL(out);
    cJSON *parsed = cJSON_Parse(out);
    TEST_ASSERT_NOT_NULL(parsed);
    cJSON *m = cJSON_GetObjectItem(parsed, "method");
    TEST_ASSERT_NOT_NULL(m);
    TEST_ASSERT_EQUAL_STRING("mirror.profile.changed", m->valuestring);
    cJSON_Delete(parsed);
    free(out);
}
