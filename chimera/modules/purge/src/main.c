...
#define PURGE_VERSION "0.1.0-alpha"

static char *build_register(void) {
    static const char *const METHODS[] = {
        "purge.status",        "purge.target.add", "purge.target.remove",
        "purge.target.list",   "purge.config.set", "purge.config.get",
        "purge.arm",           "purge.disarm",     "purge.test",
        "purge.trigger",
    };
    ...
    cJSON_AddStringToObject(p, "module", "purge");
    cJSON_AddStringToObject(p, "version", PURGE_VERSION);
...
int main(int argc, char **argv) {
    ...
    if (fd < 0) {
        fprintf(stderr, "purge: cannot connect to %s\n", socket_path);
        return 1;
    }
