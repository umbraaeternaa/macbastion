...
static char *build_heartbeat(void) {
    ...
    cJSON_AddStringToObject(params, "module", "vault");
...
static void handle_inbound(int fd, vault_runtime_t *rt, const char *line) {
    ...
    char *resp = vault_commands_dispatch(rt, req->method, req->params, req->id);
    if (resp) {
        send_frame(fd, resp);
        free(resp);
    }
